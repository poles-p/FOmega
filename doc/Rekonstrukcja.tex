...
Ponadto, przy $\beta$ redukcji typów, oraz przy typowaniu instrukcji \verb+tlet+, pojawi się podstawienie, które
podstawia typy za zmienne kwantyfikowane abstrakcją. Działa ono jak zwykłe podstawienie znane nam z~rachunku $\lambda$,
więc nie będziemy go tu szerzej omawiać. Dla odróżnienia będziemy oznaczać je przez $\{ X_1 := T_1, \dots, X_n := T_n \}$.
Oprócz podstawień postaci $\{X := T\}$, będziemy używać podstawień postaci $\{X(A) := T\}$, gdzie $A = \{ \widehat{X}_1, \dots, \widehat{X}_n \}$ 
jest zbiorem zmiennych rodzajowych.
Takie podstawienia działają podobnie, z~tą różnicą, że za wszystkie wystąpienia zmiennej $X$ podstawia $\{ \widehat{X}_1 := fresh, \dots , \widehat{X}_n := fresh \} T$,
przyczym komplet świeżych zmiennych jest inny dla każdego wystąpienia zmiennej $X$.

\begin{definicja}
\emph{Podstawieniem} nazwiemy skończony zbiór par postaci $(\bar{X},T)$, $(\bar{X},fail)$ oraz $(\widehat{X},K)$, takich że
...
\emph{Podstawieniem} pustym nazwiemy podstawienie będące zbiorem pustym i~ będziemy oznaczać przez $[]$.
Podstawienia, będziemy reprezentować jako listy postaci
\[
[\bar{X}_1 := T_1, \dots, \bar{X}_k := T_k, \bar{X}_{k+1} := fail, \dots , \bar{X}_n := fail, \widehat{X}_1 := K_1, \dots, \widehat{X}_m := K_m]
\]
\emph{Dziedziną} podstawienia $\sigma$ nazwiemy zbiór takich zmiennych typowych $\bar{X}$ i~rodzajowych $\widehat{X}$, że
$(\bar{X}, T) \in \sigma$ lub $(\bar{X}, fail) \in \sigma$ oraz $(\widehat{X}, K) \in \sigma$. Dziedzinę podstawienia $\sigma$
będziemy oznaczać przez $Dom(\sigma)$.
\end{definicja}

\begin{definicja}
Podstawienie może działać na rodzajach, typach, termach i~kontekstach. Jeżeli w~podstawieniu występuje
para $\widehat{X} := K$, to za wszystkie wystąpienia zmiennej $\widehat{X}$ zostanie podstawione $K$. Analogicznie
się dzieje w~przypadku wystąpienia pary $\bar{X} := T$. Jeżeli w~podstawieniu wystąpi para $\bar{X} := fail$,
a~w~typie/termie/kontekście do którego podstawienie aplikujemy występuje zmienna $\bar{X}$, to podstawienie jest niemożliwe.

Niech $\sigma$ będzie podstawieniem. Działanie podstawienia formalnie definiujemy jako:
\begin{align*}
\sigma * & = * \\
...
\begin{definicja}
Powiemy, że podstawienie $\rho$ jest \emph{złożeniem} podstawień $\sigma$ i~$\theta$, które będziemy oznaczać $\sigma\circ\theta$ jeżeli
...
\begin{lemat}
Niech $\sigma$ i~$\theta$ będą podstawieniami. Wówczas zachodzi
\begin{align*}
...
\end{align*}
Pozwala nam to algorytmicznie wyliczać złożenia podstawień.
\end{lemat}
\begin{proof}
\end{proof}

\begin{definicja}
\emph{Anulowaniem zbioru $A$ zmiennych typowych kwantyfikowanych abstrakcją z~podstawienia $\sigma$} nazwiemy operację
...
\end{align*}
\emph{Anulowanie zmiennej $X$ z~podstawienia $\sigma$} oznaczamy przez $\sigma \setminus X$ i~definiujemy jako $\sigma \setminus \{X\}$.
\end{definicja}
...
niech $\rho$ będzie podstawieniem takim, że $FTV(\rho) \cap A = \emptyset$ (w~szczególności zawierającym tylko zmienne rodzajowe). 
Wówczas zachodzi
\[
...
podstawieniem takim, że $FTV(\rho) \cap BTV(T_1) = \emptyset$ oraz $FTV(\rho) \cap BTV(T_2) = \emptyset$.
...
\begin{twierdzenie}
Niech $t$ i~$\Gamma$ nie zawierają zmiennych schematowych. Wówczas $typeof(\Gamma \vdash t : ?) = (\sigma, T)$ wtedy 
...
Niech $T$ i~$\Gamma$ nie zawierają zmiennych schematowych. Wówczas $kindof(\Gamma \vdash t :: ?) = (\sigma, K)$ wtedy
...
istnieje takie podstawienie $\theta$, że zachodzą następujące własności:
\begin{itemize}
...
Szkic dowodu:
\begin{enumerate}
\item Podobnie jak w~dowodzie twierdzenia \ref{twtypecheck} najpierw indukcyjnie dowodzimy alaogicznego twierdzenia
dla funkcji $kindod$ i~relacji rodzajowania.
\item Indukcyjnie po strukturze termu $t$ pokazujemy tezę. Podstawienie $\theta$ konstruujemy tak jak podstawienie
$\sigma$ tyle, że z~pominięciem alnulowania zmiennych związanych. W~miejscach w~których korzystamy z~$\beta$-unifikacji
korzystamy z~definicji $\beta$-unifikatora by zkonstruować podstawienie $\theta$.
\end{enumerate}
\end{proof}
