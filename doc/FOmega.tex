...
\subsection{Definicje lokalne}
 	...
 	\[\mathtt{let\;x=t_1\;in\;t_2 \longrightarrow (\lambda x:T.t_2)t_1 \hspace{3em}\text{gdzie}\; t_1:T 
		}
	\]
...
$\mathtt{\{^*U::K,t\}\;as\; \{\exists X::K,T\}\; \overset{def}{=} \;\lambda Y::\ast.(\lambda f:\forall X::K.T \rightarrow Y) .f\;[U]\;t}$ \\
$\mathtt{let\;\{X,x\}=t\;in\;t'\;\overset{def}{=}\;t[T'](\lambda X::K.\lambda x:T.t')}\hspace{3em} \text{gdzie $\mathtt{t':T'}$} $
