...
\begin{tabular}{| l c r |}
  ...
  $\mathtt{T}$ ::= & $\cdots$ & \textit{typy}  \\
   & $\mathtt{\{\exists X,T\}}$ & \textit{typ egzystencjalny} \\
  \hline
\end{tabular} \\
...
Na pierwszy rzut oka termy te są niezrozumiałe. Pokażę, że zachodzą podstawowe własności pakowania i odpakowania. \\
...
$\mathtt{ \{^*Nat,\{a=0, f:\lambda x:Nat.succ\;x\}\}\;as\;\{\$X, \{a:X, f:X \rightarrow Nat\}\}}$. \\ 

Rozważmy bardziej życiowy przykład: \\ 
\small{
$ \mathtt{ \{Nat::K,\{a=0, f:\lambda x:Nat.succ\;x\}\}\;as\; \{\exists X::K,\{a:X, f:X \rightarrow Nat\}\}= }$ \\ 
$ \mathtt{ = let\;x=\{a=0, f:\lambda x:Nat.succ\;x\}\;in\;\lambda Y::\ast.\lambda f:(\forall X::K.\{a:X, f:X \rightarrow Nat\} \rightarrow Y).f[Nat]x = }$ \\
...
$ \mathtt{ \{a=0, f:\lambda x:Nat.succ\;x\} \overset{\{a=0, f:\lambda x:Nat.succ\;x\}:[X \mapsto Nat]\{a:X, f:X \rightarrow Nat\}}{=}  }$ \\
$ \mathtt{ = \lambda Y::\ast.\lambda f:(\forall X::K.\{a:X, f:X \rightarrow Nat\} \rightarrow Y).f[Nat](\{a=0, f:\lambda x:Nat.succ\;x\}:[X \mapsto Nat]\{a:X, f:X \rightarrow Nat\})}$ \\
...
Odpakowanie otrzymanego termu pozostawiamy Czytelnikowi jako ćwiczenie :-)
