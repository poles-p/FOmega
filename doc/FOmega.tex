...
\begin{tabular}{| l c r |}
  ...
  $\mathtt{T}$ ::= & $\cdots$ & \textit{typy}  \\
   & $\mathtt{\{\$X,T\}}$ & \textit{typ egzystencjalny} \\
  \hline
\end{tabular} \\

Teraz możemy sobie zdefiniować te operacje w $F_\omega$: \\
$\mathtt{\{\$X::K,T\}\;\overset{def}{=}\;\forall Y::\ast.(\forall X::K.X \rightarrow Y) \rightarrow Y}$ \\
$\mathtt{\{^*U::K,t\}\;as\; \{\$X::K,T\}\; \overset{def}{=} \;let\;x=t\;in\;\lambda Y::\ast.(\lambda f:\forall X::K.T \rightarrow Y) .f\;[U]\;x}$ \\
...
Rozważmy term $\mathtt{ \{^*U::K,t\}\;as\; \{\$X::K,T\}}$. \\ 
$ \mathtt{ \{^*U::K,t\}\;as\; \{\$ X::K,T\} }$ \\ 
...
co jest typu $\mathtt{\forall Y::\ast.(\forall X::K.T \rightarrow Y) \rightarrow Y}$, czyli z definicji $\mathtt{\{\$X::K,T\}}$. \\

Rozważmy bardziej życiowy przykład. Oto typowanie w systemie $F$ przykładowego termu~\texttt{t}:\\ 
 	\[\mathtt{ \frac{\Gamma \vdash \{a=zero, f:\lambda x:Nat.succ\;x\}:[X \mapsto Nat]\{a:X,f:X \rightarrow Nat\}}{\Gamma \{^*X,\{a=zero, f:\lambda x:Nat.succ\;x\}\}\;as\;\{\$X,\{a:X,f:X \rightarrow Nat\}\}}
		}
	\]
Następnie wyprowadzimy ten term w $F_\omega$: \\ \\
\small{
$ \mathtt{ \{Nat::K,\{a=zero, f:\lambda x:Nat.succ\;x\}\}\;as\; \{\$ X::K,\{a:X, f:X \rightarrow Nat\}\}= }$ \\ 
...
co jest typu $\mathtt{\forall Y::\ast.(\forall X::K.\{a:X, f:X \rightarrow Nat\} \rightarrow Y) \rightarrow Y}$, czyli z definicji $\mathtt{\{\$X::K,\{a:X, f:X \rightarrow Nat\}\}}$. \\
}

Uważne odpakowanie otrzymanego termu pozostawiamy Czytelnikowi jako ćwiczenie, my pozwolimy sobie przeprowadzać schemat wywodu (po dwie $\beta$-redukcje):\\
\scriptsize{
